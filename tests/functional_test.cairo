...
fn init() -> (IspSTRKDispatcher, IERC20Dispatcher, IWithdrawalQueueNFTDispatcher) {
    ...
    (sp_strk, strk_token, withdrawal_nft)
}

#[test]
fn test_init() {
    let (spStark, _, _) = init();

    let token = erc20(spStark.contract_address);
    assert_eq!(token.total_supply(), 0);

    let ownable = IOwnableDispatcher { contract_address: spStark.contract_address };
...
#[test]
fn test_deposit() {
    let (sp_stark, strk_token, _) = init();
...
fn test_nft_transfer_and_claim() {
    ...
    let user1 = get_contract_address();
    let user2: ContractAddress = 2.try_into().unwrap();
    let amount = ether(1);
    ...
    assert_eq!(nft_token.owner_of(token_id), user2);
...
fn test_claim_unlock_with_nft_not_owner() {
    let (sp_stark, strk, nft) = init();
...
#[test]
fn test_successful_admin_deposit() {
    let (sp_stark, strk, _) = init();
...
fn test_withdraw_low_funds() {
    let (sp_stark, _, _) = init();
    sp_stark.admin_withdraw(ether(1));
}
...
fn test_successful_withdraw() {
    ...
    sp_stark.admin_withdraw(withdraw_amount);
...
fn test_withdraw_with_liquidity_reserve() {
    ...
    sp_stark.admin_withdraw(ether(50));
}
...
fn test_withdraw_exceeds_liquidity_reserve() {
    ...
    sp_stark.admin_withdraw(ether(51));
}
...
// Validator tests - just add _ for NFT parameter
#[test]
fn test_stake_with_auto_delegation() {
    ...
    let expected_buffer = (stake_amount * 10) / 100;
...
fn test_claim_validator_rewards_no_validator() {
    ...
    let zero_address: ContractAddress = 0.try_into().unwrap();
    let withdrawal_nft = deploy_withdrawal_nft(zero_address);
    ...
    let sp_strk = deploy_contract(
        InitParams {
            ...
            unlock_period: 60,
            claim_window: 604800,
            withdrawal_queue_nft: withdrawal_nft.contract_address,
        },
    );
